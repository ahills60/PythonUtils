...
import cPickle, os, sys, bz2
from scipy import io

def write(filename, data, overwrite=False, compress=False):
    """
    ...
    if filename.lower().endswith('.mat'):
        ...
        try:
            if compress:
                ...
                myFile = bz2.BZ2File(filename, 'w')
                myFile.write(cPickle.dumps(data))
            else:
...
def read(filename):
    ...
    if filename.lower().endswith('.mat'):
        ...
        # Attempt to open with gzip first
        try:
            myFile = bz2.BZ2File(filename, 'r')
            output = cPickle.loads(myFile.read())
            myFile.close()
            return output
        except IOError, e:
            # Most likely bounced as it's not a gzip type.
            myFile.close()
